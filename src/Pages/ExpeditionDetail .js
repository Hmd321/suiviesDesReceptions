...
import {
    ...
    TableHead, TableRow, Paper, Box, CircularProgress, Alert,
    Card, CardContent, Grid, Button, TextField
} from '@mui/material';

const ExpeditionDetail = ({ expeditionId }) => {
    ...
    useEffect(() => {
        const fetchExpedition = async () => {
            try {
                const response = await axios.get(`http://localhost:3000/expeditions/${expeditionId}`);
                ...
                setFormData({
                    ...
                    depotArrivee: response.data.depotArrivee
                });
                setProducts(response.data.products);
                console.log(response.data.products)
            } catch (error) {
                setError('Error fetching expedition data');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleCancelClick = () => {
        setIsEditing(false);
        setFormData({
            numCommande: expedition.numCommande,
            numTransfert: expedition.numTransfert,
            date: expedition.date,
            nomChauffeur: expedition.nomChauffeur,
            immatriculationCamion: expedition.immatriculationCamion,
            depotArrivee: expedition.depotArrivee
        });
        setProducts(expedition.products);
    ...
    const handleSaveClick = async () => {
        ...
        products.forEach((product) => {
            if (product.quantityShipped < 0 || product.quantityAnomaly < 0) {
                isValid = false;
        ...
        try {
            await axios.put(`http://localhost:3000/expeditions/${expeditionId}`, {
                ...formData,
                products
            });
            setExpedition({
            ...
            setError('Error updating expedition data');
        }
    };
                ...
                <Grid container spacing={2}>
                                ...
                                {isEditing ? (
                                    <>
                                        ...
                                        <TextField
                                            fullWidth
                                            margin="normal"
                                            label={t('depotArrivee')}
                                            name="depotArrivee"
                                            value={formData.depotArrivee}
                                            onChange={(e) => setFormData({ ...formData, depotArrivee: e.target.value })}
                                        />
                                        <Box mt={2}>
                                    ...
                                    <>
                                        <Typography variant="h6" gutterBottom>
                                            <strong>{t('commandeNumber')}:</strong> {expedition.numCommande}
                                        </Typography>
                                        <Typography variant="h6">
                                            <strong>{t('transfertNumber')}:</strong> {expedition.numTransfert}
                                        </Typography>
                                        <Typography variant="h6">
                                            <strong>{t('date')}:</strong> {new Date(expedition.date).toLocaleDateString()}
                                        </Typography>
                    ...
                    <Grid item xs={12} sm={6}>
                        <Card variant="outlined">
                            <CardContent>
                                <Typography variant="h6" gutterBottom>
                                    <strong>{t('chauffeurName')}:</strong> {expedition.nomChauffeur}
                                </Typography>
                                <Typography variant="h6">
                                    <strong>{t('camionRegistration')}:</strong> {expedition.immatriculationCamion}
                                </Typography>
                                <Typography variant="h6">
                                    <strong>{t('depotArrivee')}:</strong> {expedition.depotArrivee}
                                </Typography>
                            </CardContent>
                        </Card>
                    </Grid>
                        ...
                        <Table>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell><strong>{t('quantityShipped')}</strong></TableCell>
                            ...
                            <TableBody>
                                {products.map((product, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>
                                            {isEditing && productEditIndex === index ? (
                                                <TextField
                                                    fullWidth
                                                    type="number"
                                                    value={product.quantiteExpediee}
                                                    onChange={(e) => handleProductInputChange(e, index, 'quantityShipped')}
                                                />
                                            ) : (
                                                product.quantiteExpediee
                                            )}
                                        </TableCell>
                                        <TableCell>
                                            {isEditing && productEditIndex === index ? (
